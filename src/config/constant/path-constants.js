/*---------- KEYWORDS ----------*/
const SLASH = "/";

const ACCOUNT = "account";
const ACCOUNT_SETTINGS = "account-settings";

const ADMIN_PANEL = "admin-panel";

const LOGIN = "login";
const RESET_PASSWORD = "reset-password";
const REGISTER = "register";

const CREATE_EVENT = "create-event";
const FUTURE_EVENTS = "future-events";
const PAST_EVENTS = "past-events";
const EVENT_DETAILS = "event-details";

const MEMBERS = "members";
const MEMBER_DETAILS = "member-details";

const RACETRACKS = "racetracks";
const RACETRACK_DETAILS = "racetrack-details";

/*---------- PATHS ----------*/
export const PATH_ACCOUNT = SLASH + ACCOUNT;
export const PATH_ACCOUNT_SETTINGS = SLASH + ACCOUNT_SETTINGS;

export const PATH_ADMIN_PANEL = SLASH + ADMIN_PANEL;

export const PATH_LOGIN = SLASH + LOGIN;
export const PATH_RESET_PASSWORD = SLASH + RESET_PASSWORD;
export const PATH_REGISTER = SLASH + REGISTER;

export const PATH_CREATE_EVENT = SLASH + CREATE_EVENT;
export const PATH_FUTURE_EVENTS = SLASH + FUTURE_EVENTS;
export const PATH_PAST_EVENTS = SLASH + PAST_EVENTS;
export const PATH_EVENT_DETAILS = SLASH + EVENT_DETAILS;

export const PATH_HOME = SLASH;

export const PATH_MEMBERS = SLASH + MEMBERS;
export const PATH_MEMBER_DETAILS = SLASH + MEMBER_DETAILS;

export const PATH_RACETRACKS = SLASH + RACETRACKS;
export const PATH_RACETRACK_DETAILS = SLASH + RACETRACK_DETAILS;

/*---------- FIREBASE ----------*/
